  ...
  useEffect(() => {
    const fetchPosts = async () => {
      if (user) {
        ...
        const updatedGradesResult = 
          message.courseEnrollements.map((grade) => {
        ...
        axios.post(
          `/api/exams/examRes/${params.id}/result`,
          updatedGradesResult
        );
        const updatedGradesPass = message.courseEnrollements.map((grade) => {
            const student = personalInfoMessage.find(
              (student) => student.id === grade.student_id
            );
        ...
        const updatedGradesRepeat = message.courseEnrollements.map((grade) => {
            const student = personalInfoMessage.find(
              (student) => student.id === grade.student_id
            );
        ...
        axios.post(
          `/api/exams/examRes/${params.id}/can_repeat`,
          updatedGradesRepeat
        );
      }
    };
...
const handleGradeChange = (studentId: number, exam: string, grade: string) => {
  ...
  const handleSubmit = (name: string) => {
    ...
    axios
      .post(`/api/exams/examRes/${params.id}/${name}`, grades?.courseEnrollements)
      .then(() => {
