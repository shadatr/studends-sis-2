...
const Page = ({ params }: { params: { id: number } }) => {
  ...
  const [selectedMajorCourse, setSelectedMajorCourse] = useState<
    AddCourseType[]
  >([]);
  const [sections, setSections] = useState<SectionType[]>([]);
  const [selectedSections, setSelecetedSections] = useState<SectionType[]>([]);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      if (user) {
        ...
        const selectedMajorCourse = messageCour.filter((item1) =>
          majorCourses.find((item2) => item1.id === item2.course_id)
        );
        setSelectedMajorCourse(selectedMajorCourse);
        ...
        setSelecetedSections(selectedSections);
      }
    };
    fetchPosts();
  }, [params, user, loadCourses, edit, selectedCourse]);
  ...
  return (
    <div className="flex flex-col absolute w-[90%]  items-center justify-center text-[16px]">
      ...
      {activeTab === 'Tab 1' && (
        <>
          <div className="flex flex-row-reverse items-center justify-center  w-[100%] m-10 ">
            <select
              id="dep"
              dir="rtl"
