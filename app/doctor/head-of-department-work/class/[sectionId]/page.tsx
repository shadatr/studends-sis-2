...
const Tabs = ({ params }: { params: { sectionId: number } }) => {
  ...
  return (
    <div className="flex lg:absolute flex-col justify-center items-center lg:w-[80%] sm:w-[100%]">
      <Link
        href={`/doctor/head-of-department-work/courseStudents/${params.sectionId}`}
        className="p-2 rounded-md text-secondary bg-blue-700 hover:bg-blue-500 text-sm"
      >
        الطلاب و الدرجات
      </Link>
      <button
        className="m-5  bg-blue-500 hover:bg-blue-600  text-secondary p-3 rounded-md lg:w-[200px] sm:w-[100px]"
        type="submit"
        ...
        editClasses.map((item2) => {
          ...
          return (
            <div key={index} className="mb-10">
              <table className="courseInfo lg:text-sm sm:text-[8px] lg:w-[500px] sm:[350px] m-10">
                {edit ? (
                  <tbody>
                    <tr>
