...
const page = () => {
    ...
    const titles = stuInfo.map((title, index) => (
      <td className='flex justify-center p-2 items-center text-right' key={index}>{title.header}</td>
    ));
    const data = useMyData.map((item, index) => (
      <tr key={1} className="flex flex-col">
        <td className='p-2' key={index}>{item.name}</td>
        <td className='p-2' key={index}>{item.surname}</td>
        <td className='p-2' key={index}>{item.birth_date}</td>
        <td className='p-2' key={index}>{item.address}</td>
        <td className='p-2' key={index}>{item.phone}</td>
        <td className='p-2' key={index}>{item.email}</td>
        <td className='p-2' key={index}>{item.enrollment_date}</td>
      </tr>
    ));
